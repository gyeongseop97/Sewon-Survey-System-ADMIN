...
async function loadSubmittedAnswersTable(){
  ...
  if (!surveyId){
    tbody.innerHTML = `<tr><td colspan="9" class="muted">설문 정보가 없습니다.</td></tr>`;
    return;
  }
...
return Math.round((res?.totalScore || 0) * 10) / 10;
  ...
  if (rows.length === 0){
    tbody.innerHTML = `<tr><td colspan="9" class="muted">제출된 답변이 없습니다.</td></tr>`;
    return;
  }
...
for (let base=0; base<(g1Rows||[]).length; base+=colsPerRow){
  ...
  chunk.forEach((g, i)=>{
    ...
    const show = isExcluded ? "배점제외" : fmtPoint(parseFloat(String(v).replace(/[^0-9.+-]/g,'')) ); // ✅ x.x점 통일
