...
window.addEventListener("DOMContentLoaded", async () => {
  try {
    // ✅ 로그인 모달 실행 → 성공하면 session 리턴 받기
    const session = await requireLoginOrModal();
    if (!session) return; // 로그인 안 했으면 여기서 종료 (화면 안 띄움)
    bindTop();

    // ✅ 로그인 직후: ‘설문 관리’의 “내 설문 리스트”를 먼저 로드
    // (HTML에 tbody id가 없으면 조용히 무시됨)
    try { await loadMySurveys(); } catch (e) { console.warn(e); }

    // ✅ 로그인 성공 시, ‘설문 관리’의 내 설문 리스트를 즉시 불러옵니다.
