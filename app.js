...
async function openSubmittedAnswers(survey){
  ...
  if (bulkBtn){
    bulkBtn.title = "제출된 응답 중 아직 결과 전송을 하지 않은 항목을 한 번에 전송합니다.";
  ...
  tbody.querySelectorAll(".btn-send-result-response").forEach(btn => {
    btn.addEventListener("click", async () => {
      ...
      try {
        ...
        if (error) throw error;
        const sj = (data && data.submitted_json) ? data.submitted_json : {};
  ...
  if (bulkBtn && !bulkBtn.__bound){
    ...
    bulkBtn.addEventListener("click", async () => {
      try{
        ...
        if (!targets.length){
          alert("전송할 대상이 없습니다. (이미 모두 전송됨)");
          return;
        }

        if (!confirm(`아직 결과 전송되지 않은 ${targets.length}건을 일괄 전송할까요?`)) return;
