...
function renderMySurveyList(){
  ...
  rows.forEach((s, i) => {
    ...
    tr.innerHTML = `
      ...
      <td><div><code>${escapeHtml(s.code || "")}</code></div><div class="muted" style="font-size:12px;margin-top:4px;">기간: ${formatWindowText(s)}</div></td>
       <td>
         <button class="btn" data-act="edit" data-id="${s.id}">편집</button>
         <button class="btn" style="margin-left:6px;" data-act="window" data-id="${s.id}">기간설정</button>
       </td>
