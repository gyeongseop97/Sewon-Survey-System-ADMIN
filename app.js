...
window.sb = window.supabase.createClient(SUPABASE_URL, SUPABASE_KEY, {
  auth: {
    ...
    storageKey: "sb-pztlmyfutfmbmlvavwuz-auth-token",
  },
});
const sb = window.sb;
...
async function loadMySurveys(){
  ...
  const { data, error } = await sb
    .from("surveys")
    .select("id, title, code, owner_id, created_at")
...
function renderMySurveyList(){
  ...
  rows.forEach((s, i) => {
    ...
    tr.innerHTML = `
      ...
      <td><button class="btn" data-act="edit" data-id="${s.id}">편집</button></td>
      <td><button class="btn" data-act="answers" data-id="${s.id}">제출된 답변 보기</button></td>
      <td><button class="btn danger" data-act="delete" data-id="${s.id}">삭제</button></td>
...
async function loadSubmittedAnswersTable(){
  ...
  tbody.innerHTML = `<tr><td colspan="8" class="muted">불러오는 중...</td></tr>`;

  const surveyId = currentAnswersSurvey?.id;
  if (!surveyId){
    tbody.innerHTML = `<tr><td colspan="8" class="muted">설문 정보가 없습니다.</td></tr>`;
    return;
  }
...
return Math.round((res?.totalScore || 0) * 10) / 10;
  ...
  if (error){
    tbody.innerHTML = `<tr><td colspan="8" class="muted">조회 오류: ${error.message}</td></tr>`;
    return;
  }
  ...
  if (rows.length === 0){
    tbody.innerHTML = `<tr><td colspan="8" class="muted">제출된 답변이 없습니다.</td></tr>`;
    return;
  }
  ...
  const getMeta = (obj, keys) => {
    // ✅ USER 앱 저장 포맷: { target:{company,name}, answers:{...}, savedAt }
    const t = obj?.target || {};
    for (const k of keys){
      if (t && t[k] != null && t[k] !== "") return t[k];          // target 우선
      if (obj && obj[k] != null && obj[k] !== "") return obj[k];  // 그 다음 기존 키
    }
    return "";
  };
...
rows.forEach((r, idx) => {
  ...
  const company = p.company_name
    || getMeta(submitted, ["company_name", "companyName", "회사명"])
    || "-";
  ...
  tbody.querySelectorAll(".btn-edit-response").forEach(btn => {
    btn.addEventListener("click", async () => {
      ...
      try{
        ...
        state.sim.company = submitted.company || "";
