...
const EVIDENCE_DOWNLOAD_URL = "https://sewon-upload.mgs15158.workers.dev/file/";

// JSZip이 있으면 ZIP 사용, 없으면 개별 다운로드 fallback
function getEvidenceFilesFromSubmitted(submitted){
  if (!submitted || typeof submitted !== "object") return {};
  return submitted.evidenceFiles || {};
}

function getEvidenceFilesForQuestion(submitted, qid){
  const map = getEvidenceFilesFromSubmitted(submitted);
  return Array.isArray(map[qid]) ? map[qid] : [];
}

function buildEvidenceDownloadUrl(storedName){
  return EVIDENCE_DOWNLOAD_URL + encodeURIComponent(storedName || "");
}

function downloadEvidenceFile(fileMeta){
  if (!fileMeta?.storedName) {
    alert("저장된 파일 정보가 없습니다.");
    return;
  }

  const a = document.createElement("a");
  a.href = buildEvidenceDownloadUrl(fileMeta.storedName);
  a.download = fileMeta.originalName || fileMeta.storedName || "evidence";
  document.body.appendChild(a);
  a.click();
  a.remove();
}

async function fetchEvidenceBlob(fileMeta){
  const res = await fetch(buildEvidenceDownloadUrl(fileMeta.storedName));
  if (!res.ok) throw new Error(`파일 다운로드 실패: ${fileMeta.originalName || fileMeta.storedName}`);
  return await res.blob();
}

async function downloadEvidenceZipForResponse(responseRow){
  ...
  if (!allFiles.length) {
    alert("첨부된 증빙자료가 없습니다.");
    return;
  }

if (!window.JSZip) {
  alert("ZIP 라이브러리(JSZip)가 로드되지 않았습니다. index.html에 JSZip 스크립트를 추가해 주세요.");
  return;
}

  const zip = new JSZip();
  for (const f of allFiles) {
    const blob = await fetchEvidenceBlob(f);
    zip.file(f.originalName || f.storedName || "evidence", blob);
  }
...
async function downloadBulkEvidenceZip(rows){
  ...
  if (!window.JSZip) {
    alert("JSZip이 없어 일괄 ZIP 다운로드를 할 수 없습니다.");
    return;
  }
  ...
  for (const r of validRows) {
    ...
    const folder = zip.folder(`response_${r.id}`);
    for (const f of allFiles) {
      const blob = await fetchEvidenceBlob(f);
      folder.file(f.originalName || f.storedName || "evidence", blob);
    }
  }
  ...
  const a = document.createElement("a");
  a.href = URL.createObjectURL(zipBlob);
  a.download = `evidence_bulk_${currentAnswersSurvey?.code || "survey"}.zip`;
  document.body.appendChild(a);
  a.click();
  a.remove();
  URL.revokeObjectURL(a.href);
...
const defaultSurvey = () => ({
  ...
  evidenceSettings: {
    ...
    allowedExt: ["pdf", "jpg", "jpeg", "png", "xlsx", "xls", "docx", "doc", "zip"],
    totalSurveyLimitMB: 1024
  },
  groups1: [],
  rules: []
});
...
function ensureSurveyEvidenceSettings(survey) {
  survey.evidenceSettings = survey.evidenceSettings || {};
  const es = survey.evidenceSettings;

  if (es.enabled === undefined) es.enabled = true;
  if (es.defaultMaxFiles === undefined) es.defaultMaxFiles = 5;
  if (es.defaultMaxSizeMB === undefined) es.defaultMaxSizeMB = 200;
  if (!Array.isArray(es.allowedExt)) {
    es.allowedExt = ["pdf", "jpg", "jpeg", "png", "xlsx", "xls", "docx", "doc", "zip"];
  }
  if (es.totalSurveyLimitMB === undefined) es.totalSurveyLimitMB = 1024;
}
function ensureQuestionSpec(q) {
  ...
  // 문항별 증빙 설정: on/off + 필수 여부만
  if (!q.evidence) q.evidence = {};
  ...
  const toItemCheck = (label) => ({
    kind: "CHECK",
    label: String(label || "").trim() || "옵션",
    placeholder: ""
  });

  const toItemText = (label, ph) => ({
    kind: "TEXT",
    label: String(label || "").trim() || "주관식",
  ...
  if (as.mode === "TEXT_MULTI") {
    if (!Array.isArray(as.items) || !as.items.length) {
      as.items = [toItemText("주관식", "")];
    }
    as.items = as.items
  ...
  // ------------------ Render root ------------------
function render() {
  ensureSurveyEvidenceSettings(state.survey);
...
toggle.onclick = (e) => {
  ...
  if (kind === "g1") {
    nextCollapsed = !state.ui.treeCollapsed.g1[id];
    state.ui.treeCollapsed.g1[id] = nextCollapsed;
  } else if (kind === "g2") {
    nextCollapsed = !state.ui.treeCollapsed.g2[id];
    state.ui.treeCollapsed.g2[id] = nextCollapsed;
  } else {
    return;
  }
...
for (const g1 of state.survey.groups1) {
  ...
  const g1Node = document.createElement("div");
  g1Node.className = `tree-node tree-node-g1 ${g1Collapsed ? "collapsed" : ""}`;
  g1Node.setAttribute("data-tree-node", g1.id);
  ...
  const g1Children = document.createElement("div");
  g1Children.className = "tree-children";

  const g1ChildrenInner = document.createElement("div");
  g1ChildrenInner.className = "tree-children-inner";

  for (const g2 of g1.groups2 || []) {
    const g2Collapsed = isCollapsed("g2", g2.id);

    const g2Node = document.createElement("div");
    g2Node.className = `tree-node tree-node-g2 ${g2Collapsed ? "collapsed" : ""}`;
    g2Node.setAttribute("data-tree-node", g2.id);
    ...
    const qChildrenInner = document.createElement("div");
    qChildrenInner.className = "tree-children-inner";

    (g2.questions || []).forEach((q, idx) => {
      qChildrenInner.appendChild(
        makeRow({
          level: 2,
          kind: "q",
          id: q.id,
    ...
    qChildren.appendChild(qChildrenInner);
    g2Node.appendChild(g2Row);
    g2Node.appendChild(qChildren);

    g1ChildrenInner.appendChild(g2Node);
  }

  g1Children.appendChild(g1ChildrenInner);
  g1Node.appendChild(g1Row);
  g1Node.appendChild(g1Children);

  tree.appendChild(g1Node);
}
...
function renderEditor() {
  ...
  if (mode === "edit") {
if (!state.selected) {
  ed.appendChild(renderSurveyEvidenceSettingsCard());
...
function renderSurveyEvidenceSettingsCard() {
  ensureSurveyEvidenceSettings(state.survey);
  ...
  wrap.innerHTML = `
    <b>증빙자료 공통 설정</b>
    <div class="hint" style="margin-top:6px;">설문 전체 문항에 공통 적용됩니다.</div>

    <div style="margin-top:12px;">
      <label style="display:flex; align-items:center; gap:8px;">
        <input type="checkbox" id="survey_ev_enabled" ${es.enabled ? "checked" : ""}>
        <span>증빙자료 기능 사용</span>
      </label>
    </div>

    <div style="margin-top:10px;">
      기본 최대 파일 수
      <input id="survey_ev_maxFiles" type="number" min="1" value="${Number(es.defaultMaxFiles || 5)}" style="width:100px; margin-left:8px;">
    </div>

    <div style="margin-top:10px;">
      기본 문항당 최대 용량(MB)
      <input id="survey_ev_maxSizeMB" type="number" min="1" value="${Number(es.defaultMaxSizeMB || 200)}" style="width:120px; margin-left:8px;">
    </div>

    <div style="margin-top:10px;">
      허용 확장자
      <input id="survey_ev_allowedExt" type="text" value="${escapeAttr((es.allowedExt || []).join(","))}" style="width:80%; margin-left:8px;">
      <div class="hint" style="margin-top:4px;">쉼표(,)로 구분하세요. 예: pdf,jpg,png,xlsx</div>
    </div>

    <div style="margin-top:10px;">
      설문 전체 최대 용량(MB)
      <input id="survey_ev_totalLimitMB" type="number" min="1" value="${Number(es.totalSurveyLimitMB || 1024)}" style="width:120px; margin-left:8px;">
    </div>

    <div style="margin-top:14px;">
      <button class="btn" id="survey_ev_save">공통 설정 저장</button>
    </div>
  `;

  wrap.querySelector("#survey_ev_save").onclick = () => {
    es.enabled = !!wrap.querySelector("#survey_ev_enabled")?.checked;
    es.defaultMaxFiles = Number(wrap.querySelector("#survey_ev_maxFiles")?.value || 5);
    es.defaultMaxSizeMB = Number(wrap.querySelector("#survey_ev_maxSizeMB")?.value || 200);
    es.allowedExt = String(wrap.querySelector("#survey_ev_allowedExt")?.value || "")
      .split(",")
      .map(x => x.trim().toLowerCase())
      .filter(Boolean);
    es.totalSurveyLimitMB = Number(wrap.querySelector("#survey_ev_totalLimitMB")?.value || 1024);
...
function renderQListInG2(g1, g2) {
  ...
  const sumPts = alloc === "MANUAL"
    ? (g2.questions || []).reduce((a, q) => a + Number(q.points || 0), 0)
    : 100;
  ...
  wrap.innerHTML = `
    ...
    <div class="q-add-box" style="margin-top:10px; padding:10px; border:1px solid rgba(255,255,255,.12); border-radius:12px;">
      ...
      <div style="margin-top:10px; padding:10px; border:1px dashed rgba(255,255,255,.14); border-radius:12px;">
        <div class="hint"><b>증빙자료 설정</b></div>

        <div style="margin-top:8px;">
          <label style="display:flex; align-items:center; gap:8px;">
            <input type="checkbox" id="new_q_evidence_enabled">
            <span>증빙자료 첨부 허용</span>
          </label>
        </div>

        <div style="margin-top:6px;">
          <label style="display:flex; align-items:center; gap:8px;">
            <input type="checkbox" id="new_q_evidence_required">
            <span>증빙자료 필수</span>
          </label>
        </div>
      </div>
  ...
  function renderNewModePanel() {
    ...
    const drawItems = (listEl) => {
      ...
      if (!items.length) {
        listEl.innerHTML = `<div class="hint">추가된 항목이 없습니다.</div>`;
        return;
      }
    ...
    if (mode === "YES_NO") {
      ...
      newModePanel.querySelector("#new_item_add").onclick = () => {
        ...
        labelInp.value = "";
        if (newModePanel.querySelector("#new_item_is_text")) {
          newModePanel.querySelector("#new_item_is_text").checked = false;
        }
        drawItems(listEl);
      };
      return;
  ...
  wrap.querySelector("#new_q_add").onclick = () => {
    ...
    const q = {
      ...
      scoreEnabled,
      evidence: {
        enabled: !!wrap.querySelector("#new_q_evidence_enabled")?.checked,
        required: !!wrap.querySelector("#new_q_evidence_required")?.checked
      },
      answerSpec: { mode }
    };
    ...
    wrap.querySelector("#new_q_guide").value = "";
    wrap.querySelector("#new_q_req").value = "false";
    wrap.querySelector("#new_q_evidence_enabled").checked = false;
    wrap.querySelector("#new_q_evidence_required").checked = false;
    if (alloc === "MANUAL" && wrap.querySelector("#new_q_points")) {
      wrap.querySelector("#new_q_points").value = "";
    }
  ...
  function drawQuestions() {
    ...
    g2.questions.forEach((q, idx) => {
      ...
      card.id = `qcard_${q.id}`;
      card.className = `qcard ${isCollapsed ? "collapsed" : ""}`;
      ...
      card.innerHTML = `
        <div style="display:flex; align-items:center; justify-content:space-between; gap:10px; flex-wrap:wrap;">
          <div style="display:flex; align-items:center; gap:10px; flex-wrap:wrap;">
            ...
            ${q.evidence?.enabled ? `<span class="pill">증빙첨부</span>` : ``}
            ${q.evidence?.required ? `<span class="pill">증빙필수</span>` : ``}
          </div>
          <div style="display:flex; gap:8px; flex-wrap:wrap;">
...
<div class="qcard-body" data-body="${q.id}" style="margin-top:10px;">
  <div class="qcard-body-inner" data-body-inner="${q.id}"></div>
</div>
      `;

      const body = card.querySelector(`[data-body-inner="${q.id}"]`);
      ...
      const draft2 = {
        ...
        evidence: JSON.parse(JSON.stringify(q.evidence || {
          enabled: false,
          required: false
        })),
      ...
      function renderModePanel2(panel) {
        ...
        const drawItems = (listEl) => {
          const items = Array.isArray(as.items) ? as.items : (as.items = []);
          if (!items.length) {
            listEl.innerHTML = `<div class="hint">추가된 항목이 없습니다.</div>`;
            return;
          }
          ...
          listEl.innerHTML = items.map((it, i) => {
            ...
            return `
              <div style="padding:6px 0; border-bottom:1px dashed var(--line);">
                <div style="display:flex; align-items:center; gap:10px; flex-wrap:wrap;">
                  <div style="min-width:220px;">• <b>${label}</b>
                    <span class="pill">${isText ? "주관식" : (hasText ? "체크+주관식" : "체크")}</span>
                  </div>
                  ${mode === "YES_NO" && !isText ? `
                    <label style="font-size:13px;">
                      <input type="checkbox" data-k="it_withText_${i}" ${hasText ? "checked" : ""}> 체크+주관식
        ...
        const bindYesNoInputs = () => {
          const yes = panel.querySelector(`input[data-k="yesLabel"]`);
          const no = panel.querySelector(`input[data-k="noLabel"]`);
          if (yes) yes.oninput = (e) => (as.yesLabel = e.target.value);
          if (no) no.oninput = (e) => (as.noLabel = e.target.value);
        };
        ...
        if (mode === "YES_NO") {
          ...
          panel.querySelector(`button[data-k="itemAdd"]`).onclick = () => {
            ...
            labelInp.value = "";
            if (panel.querySelector(`input[data-k="itemIsText"]`)) {
              panel.querySelector(`input[data-k="itemIsText"]`).checked = false;
            }
            drawItems(listEl);
          };
          return;
      ...
      body.innerHTML = `
        ...
        <div style="margin-top:10px; padding:10px; border:1px dashed rgba(255,255,255,.14); border-radius:12px;">
          ...
          <div style="margin-top:8px;">
            <label style="display:flex; align-items:center; gap:8px;">
              <input type="checkbox" data-k="evidence_enabled" ${draft2.evidence?.enabled ? "checked" : ""}>
              <span>증빙자료 첨부 허용</span>
            </label>
          </div>
          ...
          <div style="margin-top:6px;">
            <label style="display:flex; align-items:center; gap:8px;">
              <input type="checkbox" data-k="evidence_required" ${draft2.evidence?.required ? "checked" : ""}>
              <span>증빙자료 필수</span>
            </label>
          </div>
        </div>
      ...
      const evEnabled = body.querySelector(`input[data-k="evidence_enabled"]`);
      const evRequired = body.querySelector(`input[data-k="evidence_required"]`);
      ...
      if (evEnabled) evEnabled.onchange = (e) => (draft2.evidence.enabled = !!e.target.checked);
      if (evRequired) evRequired.onchange = (e) => (draft2.evidence.required = !!e.target.checked);
      ...
      body.querySelector(`button[data-save="${q.id}"]`).onclick = () => {
        ...
        if ((g2.scoring.questionAllocation || "EQUAL") === "MANUAL") {
          q.points = Number(draft2.points || 0);
        }
        q.scoreEnabled = !!draft2.scoreEnabled;
        q.evidence = draft2.evidence;
...
const toggleBtn = card.querySelector(`button[data-toggle="${q.id}"]`);
if (toggleBtn) {
  toggleBtn.classList.add("qcard-toggle-btn");
  toggleBtn.onclick = () => {
...
async function updateSubmittedResponseOnServer(rid, newSubmitted, totalScore){
  ...
  async function getAuthHeaders(){
    ...
    try{
      ...
      console.warn("[responses] session token lookup failed", e);
    }
  ...
  async function patchResponsesByQuery(label, query, body){
    const url = `${SUPABASE_URL}/rest/v1/responses?${query}`;
    ...
    if (!res.ok){
      ...
      console.error(`[responses] ${label} REST PATCH failed`, res.status, msg);
      throw new Error(`${label} 저장 실패: ${res.status} ${msg}`);
    }

    console.log(`[responses] ${label} REST PATCH ok`);
    return true;
  }

  // 1차: id 기준 저장. Supabase JS 체인(.eq) 오류를 피하기 위해 REST PATCH 사용.
  await patchResponsesByQuery(
    "id",
    `id=eq.${encodeURIComponent(rid)}`,
    payload
  );

  // 구버전 호환: answers 컬럼도 있으면 함께 갱신 시도
  // answers 컬럼이 없으면 400이 날 수 있으므로 경고만 남김.
  try{
    await patchResponsesByQuery(
      "answers(optional)",
      `id=eq.${encodeURIComponent(rid)}`,
...
function renderEvidenceLinksHtml(qid){
  const files = Array.isArray(state.sim?.evidenceFiles?.[qid]) ? state.sim.evidenceFiles[qid] : [];
  if (!files.length) {
    return `<div class="hint" style="margin-top:6px;">첨부된 증빙자료 없음</div>`;
  }

  return `
    <div style="margin-top:8px;">
      <div class="hint"><b>업로드된 증빙자료</b></div>
      <div style="margin-top:6px; display:flex; flex-direction:column; gap:6px;">
        ${files.map((f, i) => `
          <div style="padding:8px 10px; border:1px solid rgba(255,255,255,.12); border-radius:10px; display:flex; align-items:center; justify-content:space-between; gap:8px; flex-wrap:wrap;">
            <div>
              <div style="font-weight:700;">${escapeHtml(f.originalName || f.storedName || "evidence")}</div>
              <div class="hint">${((Number(f.size || 0)/1024/1024).toFixed(2))}MB</div>
            </div>
            <button class="btn small" type="button" data-sim-evi-qid="${escapeAttr(qid)}" data-sim-evi-idx="${i}">다운로드</button>
          </div>
        `).join("")}
      </div>
    </div>
...
<div id="sim_evidence_${q.id}" style="margin-top:10px;"></div>
...
<div style="margin-top:10px; padding-top:10px; border-top:1px dashed rgba(255,255,255,.12);">
    ...
    const evidenceHost = block.querySelector(`#sim_evidence_${CSS.escape(q.id)}`);
if (evidenceHost) {
  evidenceHost.innerHTML = renderEvidenceLinksHtml(q.id);
}

block.querySelectorAll(`[data-sim-evi-qid="${CSS.escape(q.id)}"]`).forEach(btn => {
  btn.onclick = () => {
    const qid = btn.getAttribute("data-sim-evi-qid");
    const idx = Number(btn.getAttribute("data-sim-evi-idx"));
    const files = Array.isArray(state.sim?.evidenceFiles?.[qid]) ? state.sim.evidenceFiles[qid] : [];
    const fileMeta = files[idx];
    if (!fileMeta) return;
    downloadEvidenceFile(fileMeta);
  };
});
...
const bulkEvidenceBtn = document.getElementById("btnBulkEvidenceZip");
if (bulkEvidenceBtn){
...
async function loadSubmittedAnswersTable(){
  ...
  const bulkEvidenceBtn = document.getElementById("btnBulkEvidenceZip");
if (bulkEvidenceBtn) {
  bulkEvidenceBtn.onclick = async () => {
    try{
      await downloadBulkEvidenceZip(rows);
    } catch(e){
      console.error(e);
      alert("증빙 일괄 ZIP 다운로드 실패: " + (e?.message || e));
    }
  };
}
...
tbody.querySelectorAll(".btn-dl-evidence-response").forEach(btn => {
  btn.addEventListener("click", async () => {
    const rid = btn.dataset.rid;
    const found = rows.find(x => x.id === rid);
    if (!found) return alert("응답 데이터를 찾지 못했습니다.");

    try{
      await downloadEvidenceZipForResponse(found);
    } catch(e){
      console.error(e);
      alert("증빙 ZIP 다운로드 실패: " + (e?.message || e));
    }
  });
});
  // ✅ 일괄 전송(아직 전송 안 된 응답만)
